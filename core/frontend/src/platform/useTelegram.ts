...
export default function useTelegram(): useTelegramComposableState {
  ...
  const debugMainButton = ref<HTMLButtonElement | null>(null);
  const debugBackButton = ref<HTMLButtonElement | null>(null);
   ...
   */
  function prepareDebugButton(reference: Ref<HTMLButtonElement | null>, className: string): void {
  ...
  function shareMessage(msgId: string, callback?: (success: boolean) => void): void {
